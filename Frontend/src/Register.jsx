...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/api/routes/register", form, { withCredentials: true });
      const token = response?.data?.accessToken;
      if (token) {
        setAccessToken(token);
        navigate('/User'); // Redirect to protected route after registration
        console.log('received accessToken from server:', token);
      } else {
        console.warn('No accessToken returned from register endpoint', response.data);
      }
    } catch (error) {
  ...
  <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <div className="mt-1">
          <input id="username" type="username" name="username" required onChange={handleChange} autoComplete="username"
           value={form.username}
